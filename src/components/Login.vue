		...
		<div class="register-main">
				...
				<form action="#">
					...
					<div class="txt upwd">
						...
						<input type="password" :placeholder='isPwd&&!upwd?"密码输入错误":"请输入密码 6-12位"'
						 v-model="upwd"
						 @blur='blurUpwd'
						>
					</div>
					<p class="check">
						...
						<a href="" class="login_a">忘记密码</a>
						<a href="" class="agree_a">快速注册</a>
					</p>
						<button @click="logMsg" class="txt btn" >立即登录</button>
						<div class="txt warning_msg" v-if="isWarning_msg">手机号或密码不正确，请重新登录</div>
				</form>
		</div>
	</div>
</template>
...
export default{
	...
	methods:{
		...
		logMsg:function(){
			this.$http.post("http://127.0.0.1:3000/login",{phone:this.msg,upwd:this.upwd},{emulateJSON: true}).then(function(data){
